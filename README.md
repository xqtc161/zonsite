...
`site.zon` is embedded at compile time and tokenized using `std.zig.Tokenizer`. Each token is wrapped in a colored `<span>`. String literals that look like URLs or email addresses become clickable links. Blocks with a `// fold` comment get folded.
